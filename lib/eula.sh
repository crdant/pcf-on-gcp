...
accept_eula () {
  ...
  if [ "${accepted}" = "yes" ]; then
    eula_url=`curl -qsLf -H "Authorization: Token $PIVNET_TOKEN" $releases_url | jq --raw-output "[.releases[] | select( .version | startswith(\"$version\") ) ] | max_by(.release_date) ._links .eula_acceptance .href"`
