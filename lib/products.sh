...
stage_product () {
  product=$1
  version=$2
  base_version=`echo ${PCF_VERSION%-*}`
  login_ops_manager > /dev/null

  if [ -z "${version}" ] ; then
    available_product=`curl -qsLf --insecure "${OPS_MANAGER_API_ENDPOINT}/available_products" -H "Authorization: Bearer ${UAA_ACCESS_TOKEN}" -H "Accept: application/json" | jq --raw-output ".[] | select ( .product_version | startswith (\"$base_version\" ) ) | select ( .name == \"$product\" )"`
